# === Backend API Auth Router Interface (contracts_contracts_contracts_contracts_backend_api_auth_router_interface_interface_interface_interface) v1 ===
#  Dependencies: uuid, datetime, httpx, jwt, fastapi, sqlalchemy, backend.config, backend.database, backend.models
# FastAPI authentication router interface for GitHub OAuth 2.0 flow with JWT session management. Defines contract for login redirects, OAuth callbacks, user registration/updates, logout, and authenticated user retrieval endpoints.
...
class User:
    """Database user model (from backend.models)"""
    pass

class AsyncSession:
    """SQLAlchemy async database session (from sqlalchemy)"""
    pass

class RedirectResponse:
    """FastAPI redirect response type"""
    pass
...
# __all__ = ['JWTPayload', 'UserProfile', 'User', 'AsyncSession', 'RedirectResponse', 'Response', 'HTTPException', 'create_jwt', 'decode_jwt', 'get_current_user', 'login', 'callback', 'logout', 'me']
